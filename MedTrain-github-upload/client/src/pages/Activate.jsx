...
export default function Activate() {
  ...
  async function submit(e) {
    ...
    try {
      ...
      setError(err instanceof ApiError ? err.message : t("activate_invalid"));
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-5">
      <div className="card p-8 w-full max-w-sm">
        ...
        <h1 className="font-head text-xl font-bold text-center mb-1">{t("activate_title")}</h1>
        {!token ? (
          <p className="text-sm text-center" style={{ color: "var(--red)" }}>{t("activate_invalid")}</p>
        ) : done ? (
          <>
            <p className="text-sm txt-muted text-center mb-6">{t("activate_success")}</p>
            <button className="btn btn-primary w-full justify-center" onClick={() => navigate("/login")}>{t("login_submit")}</button>
          </>
        ) : (
          <>
            <p className="text-sm txt-muted text-center mb-6">{t("activate_body")}</p>
            <form onSubmit={submit} className="space-y-4">
              ...
              <button type="submit" className="btn btn-primary w-full justify-center" disabled={busy}>
                {busy ? t("common_loading") : t("activate_submit")}
              </button>
            </form>
          </>
        )}
